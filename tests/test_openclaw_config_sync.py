...
class TestSyncWithCredentials:
    def test_sync_perplexity(self, db, tmp_config):
        ...
        _asyncio.run(save_credential_async("u1", "perplexity", "api_key", "pplx-test-abc123xyz"))
        r = _asyncio.run(sync_user_credentials_to_openclaw(db, "u1"))
    ...
    def test_sync_multiple_providers(self, db, tmp_config):
        ...
        _asyncio.run(save_credential_async("u1", "perplexity", "api_key", "pplx-1"))
        _asyncio.run(save_credential_async("u1", "brave", "api_key", "BSA-2"))
        _asyncio.run(save_credential_async("u1", "tavily", "api_key", "tvly-3"))
    ...
    def test_dry_run_no_write(self, db, tmp_config):
        ...
        _asyncio.run(save_credential_async("u1", "perplexity", "api_key", "pplx-dry"))
        # Capture contenu avant
        before = tmp_config.read_text(encoding="utf-8")
        r = _asyncio.run(sync_user_credentials_to_openclaw(db, "u1", dry_run=True))
        assert r["ok"] is True
    ...
    def test_update_existing(self, db, tmp_config):
        ...
        _asyncio.run(save_credential_async("u1", "perplexity", "api_key", "pplx-v1"))
        _asyncio.run(sync_user_credentials_to_openclaw(db, "u1"))

        # Update la cle
        _asyncio.run(save_credential_async("u1", "perplexity", "api_key", "pplx-v2-updated"))
        r = _asyncio.run(sync_user_credentials_to_openclaw(db, "u1"))
    ...
    def test_remove_missing(self, db, tmp_config):
        import asyncio as _asyncio
        from api.credentials import save_credential_async, delete_credential_async
        from api.openclaw_config_sync import sync_user_credentials_to_openclaw

        _asyncio.run(save_credential_async("u1", "perplexity", "api_key", "pplx-will-be-deleted"))
        _asyncio.run(sync_user_credentials_to_openclaw(db, "u1"))
        _asyncio.run(delete_credential_async("u1", "perplexity", "api_key"))

        r = _asyncio.run(sync_user_credentials_to_openclaw(db, "u1", remove_missing=True))
...
class TestBackup:
    def test_backup_created(self, db, tmp_config):
        ...
        _asyncio.run(save_credential_async("u1", "brave", "api_key", "BSA-backup-test"))
        _asyncio.run(sync_user_credentials_to_openclaw(db, "u1"))
